...
// ***************************************
function reDisplay(currentNumber,tocChange,noLink,e,usuario,empresa) {
// Input parameters:
...
// If there is an event that triggered the function call: Checking the control key depending on the browser used. If it is pressed and tocChange is greater than 0 tocChange is set to 2 so the ToC schanges without automatic collapsing:
	for (ir=0; ir<parent.frames[6].tocTab.length; ir++)
	{
	  toDisplay[ir] = parent.frames[6].tocTab[ir][0].split(".").length==1;
	}//fin del for
...
// Current index indica el número del arreglo en el cual se encuetra actualmente en la navegación
	...
	for (i=0; i<parent.frames[6].tocTab.length; i++) {
		if (parent.frames[6].tocTab[i][0] == currentNumber) {
			currentIndex = i;
			break;
		}
	}
...
// currentIsExpanded = Expand/Collaps-state of the current heading:
	if (currentIndex < parent.frames[6].tocTab.length-1) {
		nextLevel = parent.frames[6].tocTab[currentIndex+1][0].split(".").length-1;
...
// Determining the new URL and target (if given) of the current heading
        theHref = (noLink) ? "" : parent.frames[6].tocTab[currentIndex][2];
	theTarget = parent.frames[6].tocTab[currentIndex][3];

// ***************************************
// 1st loop over the tocTab entries: Determining which heading to display:
// ***************************************
	for (i=1; i<parent.frames[6].tocTab.length; i++) {

// Nothing to do if the tocChange parameter is set to 0. If it is set to 1 or 2...
		if (tocChange) {
			thisNumber = parent.frames[6].tocTab[i][0];
...
// Initializing the ToC window's document and displaying the title on it's top. The ToC is performed by a HTML table:
	...
	//document.clear();
        //document.write(
        parent.frames[7].document.clear();
        parent.frames[7].document.write(
"<html>\n<head>\n<title>ToC</title>\n"+
"<style>\n"+
...
//"A.title {font-family: Verdana; font-weight:bold; font-size:" + textSizes[0] + "em; color: 585858; text-decoration:yes}\n"+
"</style>\n"+
//"<style fprolloverstyle>A:hover {color:" + rollOverColor +"}</style>\n"+
"<LINK REL=\"STYLESHEET\" TYPE=\"text/css\" HREF=\"/axis/personalizacion/"+empresa+"/"+usuario+"/"+usuario+".css\">\n"+
"</head>\n"+
"<body bgcolor=\"" + backColor + "\" background = "+ fondo +" topmargin = 0 leftmargin=5 class=color_barra>\n<table border=0 cellspacing=0 cellpadding=0>\n");
...
// This is for defining the number of columns of the ToC table and the width of the last one. The first cells of each following row shall be empty or contain the heading symbol, the last ones are reserved for displaying the heding's text:
	for (k=0; k<nCols; k++) {
		...
		parent.frames[7].document.write("<td height=1></td>");
	}
	//toc.document.write("<td width=240 height=1></td></tr>");
	//document.write("<td width=240 height=1></td></tr>");
	parent.frames[7].document.write("<td width=240 height=1></td></tr>");
...
// ***************************************
	...
	for (i=1; i<parent.frames[6].tocTab.length; i++) {
		var img=null;
		if (parent.frames[6].tocTab[i][0] == "PROCESOS" || parent.frames[6].tocTab[i][0] == "TRANSACCIONES")
                {
                   titulo = true;
                }
...
// Setting the heading's symbol depending on whether this heading is expanded or not or if it is a leaf. It is expanded if the next heading has a greater level than this one AND has to be displayed:
			if (i < parent.frames[6].tocTab.length-1) {
				nextLevel = parent.frames[6].tocTab[i+1][0].split(".").length-1;
				if (!titulo)
...
// Now writing this ToC line, i.e. a table row...:
                        ...
                        //document.writeln("<tr>");
                        parent.frames[7].document.writeln("<tr>");
...
// ...first some empty cells for the line indent depending on the level of this heading...:
			for (k=1; k<=thisLevel; k++) {
				...
				parent.frames[7].document.writeln("<td>&nbsp;</td>");
			}
...
// ...then the hading symbol and the heading text each with a javaScript link caling just this function reDisplay again:
           if (img == mas || img == menos)
		   {
		     ...
		     parent.frames[7].document.writeln("<td valign=top><a href=\"javaScript:history.go(0)\"  onMouseDown=\"parent.frames[6].reDisplay('" + thisNumber + "'," + tocBehaviour[1] + "," + tocLinks[0] + ",event,'"+usuario+"','"+empresa+"')\"><img src=\"" + img + ".png\" width=10 height=10 border=0></a></td> <td colspan=" + (nCols-thisLevel) + "><a href=\"javaScript:history.go(0)\" onMouseDown=\"parent.frames[6].reDisplay('" + thisNumber + "'," + tocBehaviour[0] + "," + tocLinks[1] + ",event,'"+usuario+"','"+empresa+"')\" class=title> " + ((showNumbers)?(thisNumber+" "):"") + parent.frames[6].tocTab[i][1] + "</font></td></tr>");
		   }
		   else
		   {
		      ...
		      if (img == null)
		      {
		      	 parent.frames[7].document.writeln("<td height=5 valign=top></td><td colspan=" + (nCols-thisLevel) + "></td></tr>");
		      	 if(parent.frames[6].tocTab[i][1]=="PROCESOS")
		      	 {
		      	   parent.frames[7].document.writeln("<td valign=top></td><td colspan=" + (nCols-thisLevel) + "><font size=\"2\" face=\"Verdana\"><b>"+ "Procesos" + "</b></font></td></tr>");
		      	 }
		      	 else
		      	 {
		      	   parent.frames[7].document.writeln("<td valign=top></td><td colspan=" + (nCols-thisLevel) + "><font size=\"2\" face=\"Verdana\"><b>"+ parent.frames[6].tocTab[i][1] + "</b></font></td></tr>");
		      	 }
		      }
		      else
		      {
		 	 ...
		 	 parent.frames[7].document.writeln("<td valign=top><a href=\"javaScript:history.go(0)\" onMouseDown=\"parent.frames[6].reDisplay('" + thisNumber + "'," + tocBehaviour[1] + "," + tocLinks[0] + ",event,'"+usuario+"','"+empresa+"')\"><img src=\"" + img + ".png\" width=10 height=10 border=0></a></td> <td colspan=" + (nCols-thisLevel) + "><a href=\"javaScript:history.go(0)\" onMouseDown=\"parent.frames[6].reDisplay('" + thisNumber + "'," + tocBehaviour[0] + "," + tocLinks[1] + ",event,'"+usuario+"','"+empresa+"')\" class=" + thisClass + "> " + ((showNumbers)?(thisNumber+" "):"") + parent.frames[6].tocTab[i][1] + "</a></td></tr>");
		      }
		   }
		}
	titulo=false;
...
// Closing the ToC table and the document
	...
	//document.close();
	parent.frames[7].document.writeln("</table>\n</body></html>");
	parent.frames[7].document.close();
...
// Setting the top or content window's location if required
    newRef = "parent.frames[6]."+theHref;
    if (theHref)
